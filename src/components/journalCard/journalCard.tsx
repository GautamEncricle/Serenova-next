...
const JournalCard = ({ link, img, subtitle, title, points }: any) => {
  return (
          ...
          <div className="journal-card-list mt-16">
            <ul className="flex flex-wrap gap-y-8 gap-x-24">
              {points.map((item: any, index: number) => (
                <li
                  key={index}
                  className="poppins-400-14 relative after:content-[''] after:absolute after:top-50p after:translate-y-50mp after:right-[-14px] after:w-[3px] after:h-[3px] after:rounded-50p after:bg-secondary-600 last:after:hidden">
                  {item.text}
                </li>
              ))}
            </ul>
